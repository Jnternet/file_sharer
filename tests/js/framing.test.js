...
test('头部字节序符合协议（大端）', () => {
  const frame = encodeDataFrame(0x01020304, 0x0001_0203_0405, new Uint8Array());
  assert.deepEqual(
    [...frame],
    [
      1,
...
test('边界值：最大 fileIndex 与 0 长度负载', () => {
  const decoded = decodeDataFrame(encodeDataFrame(0xffffffff, 0, new Uint8Array()));
...
test('大 chunkIndex 正确往返', () => {
  ...
  assert.equal(decodeDataFrame(encodeDataFrame(1, big, new Uint8Array(1))).chunkIndex, big);
});

test('编码参数校验', () => {
  for (const bad of [-1, 1.5, NaN, 0x1_0000_0000, '7', null]) {
    assert.throws(() => encodeDataFrame(bad, 0, new Uint8Array()), FrameError, `fileIndex=${bad}`);
  }
  // chunkIndex 是 u64，但 JS 侧限制在安全整数内
  for (const bad of [-1, 1.5, NaN, 2 ** 53, '7', null]) {
    assert.throws(() => encodeDataFrame(0, bad, new Uint8Array()), FrameError, `chunkIndex=${bad}`);
  }
  assert.throws(() => encodeDataFrame(0, 0, [1, 2, 3]), FrameError);
  assert.equal(encodeDataFrame(0, 0, new Uint8Array()).length, DATA_HEADER_BYTES);
  assert.equal(encodeDataFrame(0, 0x1_0000_0000, new Uint8Array()).length, DATA_HEADER_BYTES);
});
...
test('解码拒绝畸形帧', () => {
  ...
  const badKind = encodeDataFrame(0, 0, new Uint8Array());
  badKind[0] = 9;
  assert.throws(
  ...
  new DataView(hugeChunk.buffer).setBigUint64(5, 2n ** 63n, false);
  assert.throws(
...
test('解码支持带偏移的视图（DataChannel 可能给到子视图）', () => {
  const inner = encodeDataFrame(3, 9, new Uint8Array([7, 7]));
  ...
  assert.equal(decoded.fileIndex, 3);
...
test('1 MiB 负载不被复制（payload 是视图）', () => {
  ...
  const decoded = decodeDataFrame(encodeDataFrame(0, 0, payload));
